...
#if defined(CONFIG_FLYER) ||  defined(CONFIG_FLYER3D)
...
/** leave this here (after malloc(), environment and PCI are working) **/
	...
	/*Serial Number */
	    ...
	    char* e;
	    char tmp[64];
	    i = getenv_r ("serialnum", tmp, sizeof (tmp));
	    if (i > 0)
	    {
		bd->bi_serial_number = simple_strtoul (tmp, &e, 10);
	    }
	    else
		bd->bi_serial_number = 0;
	    
	    out32r(PMM0PCIHA, (unsigned long)bd->bi_serial_number);
#endif
